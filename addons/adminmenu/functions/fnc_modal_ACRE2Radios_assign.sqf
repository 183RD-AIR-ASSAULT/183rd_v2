...

{
    if (player canAdd _radio) then {
        ...
        systemChat format ["[183rd Air Assault] Added radio: %1", _radio];
    } else {
        if (getContainerMaxLoad uniform player > 0) then {
            (uniformContainer player) addItemCargoGlobal [_radio, 1];
            systemChat format ["[183rd Air Assault] Added radio (exceeds inventory capacity): %1", _radio];
        } else {
            systemChat format ["[183rd Air Assault] Couldn't add radio: %1", _radio];
        };
