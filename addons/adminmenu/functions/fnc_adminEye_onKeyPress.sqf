        ...
        if (typeOf GVAR(adminEyeSelectedObj) == "oeta_ai_wavespawn") then {
            ...
            if (count _statements > 0) then { // isTrigger.
                [{
                    ...
                    _trigger setVariable ["oeta_trigger_serialised",[_statements,triggerTimeout _trigger, +_activation]];
                    
                    _activation set[2,false];
                    _trigger setTriggerActivation _activation;
                    _trigger setTriggerStatements ["true","[thisTrigger] call oeta_adminmenu_fnc_adminEye_restoreTrigger",""];
